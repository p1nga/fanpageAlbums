 ...
 * FanpageAlbums model.
 */
class FanpageAlbumsModelphotolinks extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_FANPAGEALBUMS';
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Photolinks', $prefix = 'FanpageAlbumsTable', $config = array())
	{
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__fanpagealbums');
				$max = $db->loadResult();
